			...
			const filteredApplicant = action.payload.applicant.filter((app, i) => {
				...
				for (const key in app) {
					...
					app.skills.forEach(skill => {
						skillValidity =
							skillValidity || (typeof skill === 'string' && skill.toLowerCase().includes(state.search.value.toLowerCase()));
					});
...
const ApplicantListAO = props => {
						...
						<table className={classes.Table}>
							...
							{find ? (
								<tbody className={classes.ColumnField}>
									{displayData &&
										displayData.filter(app => app.status === filter).map((app, i) => (
											<tr key={app.id}>
												<th>{i + 1}</th>

												<th>
													<Link to={`/ap/${app.id}`} style={{ color: 'black', textDecoration: 'none' }}>
														{app.id}
													</Link>
												</th>
												...
												<th>{app.email}</th>
												<th style={app.address ? { color: 'black' } : { color: 'rgba(255,0,0,0.7)' }}>
	...
	if (!props.isLoading && !data) {
		content = <p className={classes.EmptyText}>Tidak ditemukan data akun terdaftar sebagai pelamar</p>;
	}
